...
## bulk writes
Covers `insertMany` and `updateMany` - the documents are applied in the order they are presented within the call; this is true for replication too - this is the default bulk mode "ordered bulk write". Ends execution on first document failure; those documents updated remain inserted/updated, and those documents after the failed document not processed.
