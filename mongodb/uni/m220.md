...
## Read Concerns
Read Concerns are different levels of "read isolation".  A low leve lof read isolation is a read concern which returns when data has not been written to all nodes. Lowest read concern is "local", being a return having written to primary only. The most assured read concern is "majority", having written to the primarty and most of the secondaries.
